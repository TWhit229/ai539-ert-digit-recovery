...
prior settled into. The fix --- and the actual method we use --- is to
\emph{separate} the two tugs. Instead of one variable getting both
tugs simultaneously, we introduce a second variable so that each tug
acts on its own variable alone. The two variables are then glued
together with a soft penalty.
...
the \emph{base} pipeline on 10 unseen MNIST test digits (one per class
0-9). The diffusion model was fine-tuned on the training set only;
test digits are genuinely unseen.
...
Result: \textbf{9 of 10 correct} on the base pipeline. The class-conditional
+ CFG + TV polish pipeline is strictly better at recovering image quality;
in particular, with explicit class hypotheses it avoids the wrong-digit
basin that the one held-out failure fell into.
...
nearly physically indistinguishable through 1900 boundary
measurements. That's an ambiguity of the \emph{problem}, not of our
method. With the new class-conditional pipeline, we expect this case
to resolve correctly: chains seeded to class 3 commit to a 3 from the
start.

All 9 correctly-recovered images polished to misfits between $6.4
\times 10^{-13}$ and $7.6 \times 10^{-12}$ on the base pipeline.
