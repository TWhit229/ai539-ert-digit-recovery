...
\section{Lesson 43: The result on the competition vectors}

We applied the pipeline to two vectors. The first is a noiseless
digit-5 test vector we used while building the method (and the one
the 12-combination sweep in Lesson 41 was run on). The second is the
actual Competition 2 vector, which has measurement noise added.

\textbf{Vector 1 (worked example, noiseless digit-5):}

\begin{itemize}[leftmargin=2em]
\item Recovered digit: \textbf{5}, rotated by about $7.5^\circ$.
\item Polished misfit: $\mathbf{1.83 \times 10^{-11}}$.
...
Left: the input list of $1900$ voltage readings. Right: the recovered
conductivity image, with the digit 5 clearly visible, smooth strokes,
and a flat background. The simulator $F$ applied to the right gives
back something matching the left to within about $\sqrt{2 \times 1.83
\times 10^{-11}} \approx 6 \times 10^{-6}$ per measurement.

\textbf{Vector 2 (actual Competition 2, with measurement noise):}

\begin{itemize}[leftmargin=2em]
\item Recovered digit: \textbf{2}.
\item Pre-polish misfit: $\mathbf{9.67 \times 10^{-4}}$.
\item Classifier confidence on the recovered image: $1.00$ (TinyMNIST CNN).
\end{itemize}

\begin{center}
\includegraphics[width=0.99\textwidth]{fig_v2_recovered.png}
\end{center}

\textbf{Why pre-polish, not polished?} The Competition 2 vector has
measurement noise with a floor near $10^{-3}$. All $32$ chains in the
PnP-DM-CFG run converge to misfits between $9.5 \times 10^{-4}$ and
$9.7 \times 10^{-4}$ --- they're at the noise floor. Running TV polish
on top of this drives the polished misfit \emph{up} to $1.23 \times
10^{-3}$ as TV pushes the image toward smoother solutions that fit
the data less well: the TV-polish update was tuned for the
$10^{-11}$ regime of the noiseless example, and on noisy data it
overshoots below the noise floor. The pre-polish image is the answer.

\textbf{Which chain do we report?} Selecting by lowest misfit alone
gave a chain whose recovered image is visually broken (disconnected
strokes, scattered pixels) even though its misfit was the lowest at
$9.51 \times 10^{-4}$. On noisy data, lowest misfit does not equal
cleanest image. Instead, we tally the classifier vote across the $32$
chains: $18$ chains classify as a $2$, $8$ as a $3$, $4$ as a $6$,
and one each as $5$ and $8$. We then pick the chain with the highest
classifier confidence on the majority vote ($P(\mathrm{class}=2)$),
breaking ties by misfit. The chosen chain has target class $2$,
pre-polish misfit $9.67 \times 10^{-4}$, and classifier confidence
$1.00$.

\textbf{Both results, side by side:}

\begin{center}
\includegraphics[width=0.9\textwidth]{fig_v2_vs_v1.png}
\end{center}

Same pipeline, two vectors, two regimes. The left answer is bounded
by float64 precision; the right answer is bounded by measurement
noise. The pipeline correctly identifies both digits.
...
speckle. We applied it to two vectors: a noiseless digit-5 test case
where the polish drove the misfit down to about $2 \times 10^{-11}$
with a clean, recognizable 5; and the actual Competition 2 vector,
which has measurement noise --- there the pipeline recovers a 2 at
misfit about $10^{-3}$ (the measurement-noise floor), so we skip the
polish step and report the pre-polish image.''
\end{intuition}
...
\item \textbf{Final misfits.} $1.83 \times 10^{-11}$ on the noiseless
  digit-5 worked example (polished, float64 floor); $9.67 \times
  10^{-4}$ on the actual Competition 2 vector (pre-polish, sitting at
  the measurement-noise floor; polish overshoots on noisy data).
  (Lesson 43.)
\end{itemize}
