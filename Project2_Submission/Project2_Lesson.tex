...
\begin{aside}
The pretrained MNIST DDPM we use, \texttt{1aurent/ddpm-mnist} on
Hugging Face, has about $1$ million internal weights and was trained
this way on standard MNIST for a few hours on one GPU. We just
downloaded it; we did not train it from scratch.
\end{aside}
...
once, across many random seeds.

The fix --- and the actual method we use --- is to \emph{separate} the
two tugs. Instead of one variable getting both tugs simultaneously, we
introduce a second variable so that each tug acts on its own variable
alone. The two variables are then glued together with a soft penalty.
...
\section{Lesson 20: The data step --- what objective are we minimizing?}
...
(which turns ``maximize a product of bell curves'' into ``minimize a sum
of squared deviations,'' a standard trick), and drop additive constants:
\[
\boxed{\;\;
\mathcal{L}(z)
\;=\;
...
Unpack this:
\begin{itemize}[leftmargin=2em]
\item The first term is the \emph{data misfit}: how far off the
  simulator output is from the observed measurements. Smaller is
  better.
\item The second term is the \emph{coupling penalty}: how far $z$ has
  drifted from the current $x$. Smaller is better.
\item Both terms are scaled by $\frac{1}{2 \cdot \text{variance}}$.
  This is the standard weight for a Gaussian's quadratic exponent.
  Smaller variance $\Rightarrow$ bigger weight $\Rightarrow$ stronger
  pull.
\item We use $\sigma_{\text{bg}} + z$ in the simulator because in the
  ERT physics, $\sigma = \sigma_{\text{bg}} + x$: the conductivity is
  the background plus the contrast.
\end{itemize}
...
\section{Lesson 21: Gauss-Newton step 1 --- linearize $F$}
...
\section{Lesson 22: Gauss-Newton step 2 --- set the gradient to zero}
...
\section{Lesson 23: Gauss-Newton step 3 --- rearrange to a linear system}

Take the equation from Lesson 22 and put all the $\Delta$ terms on
...
This is a $784 \times 784$ linear system in the unknown $\Delta$. The
matrix on the left is symmetric positive-definite (because $J^\top J$
is positive-semidefinite and $I$ is positive-definite, and the sum
of those is positive-definite). We solve it directly with
\texttt{torch.linalg.solve} --- on a $784 \times 784$ system that takes
a few milliseconds. Then we set
\[
z \;\leftarrow\; z + \Delta.
\]
That's one Gauss-Newton step. In our code we do 1 or 2 GN steps per
data update; the first usually does most of the work.

\textbf{What does the matrix on the left mean?} The $J^\top J$ piece
captures the curvature of the data term: in directions where $J$
``sees a lot,'' $J^\top J$ is large, and the corresponding Newton
step is small (the data is already constrained there). In directions
where $J$ ``sees little,'' $J^\top J$ is near zero, and the
$1/\sigma_n^2 \cdot I$ piece keeps the matrix invertible. The damping
is what lets us solve the system robustly even when $J^\top J$ is
ill-conditioned --- which it always is for ERT.
...
\section{Lesson 24: The prior step --- one diffusion denoise}
...
\section{Lesson 25: Multi-chain mode coverage}
...
In our problem on the competition vector, most chains converge to
$5$-shaped images, while a small minority converge to a $5$-like-$8$
shape (a $5$ with its top loop closed). The lowest-misfit chain ---
the one we keep --- is always a $5$, with pre-polish misfit
$1.84 \times 10^{-7}$ in the full $32$-chain run. The slightly-higher
misfits of the $8$-shaped chains are why we use ``pick by misfit'' to
select the winner. Below are 8 sample chains from a quick demo run:

\begin{center}
\includegraphics[width=0.99\textwidth]{fig_chains_compact.png}
\end{center}

\section{Lesson 26: One more crucial piece --- rotation augmentation}
...
takes about 20 minutes on a MacBook.
...
\section{Lesson 27: Why we still need a polish step}
...
\section{Lesson 28: Gradient descent has \emph{linear} convergence}
...
\section{Lesson 29: Newton's method has \emph{quadratic} convergence}
...
called conjugate gradient, which we'll cover in Lesson 31. First, let's
look at the contrast between GD and Newton on a picture.

\section{Lesson 30: A picture of GD vs Newton}
...
\section{Lesson 31: Conjugate gradient --- solving the system without inverting}
...
\section{Lesson 32: A worked example --- CG on a $2\times 2$ system}
...
\section{Lesson 33: Line search and damping for safety}
...
\section{Lesson 34: The polish trajectory in practice}
...
\begin{key}
\textbf{Final misfit: $\mathbf{6.16 \times 10^{-13}}$.} The polish ran
to the float64 numerical floor.
\end{key}

%==============================================================
\part*{Part VI --- The pipeline, the numbers, the explanation}
\addcontentsline{toc}{section}{Part VI --- The pipeline, the numbers, the explanation}
%==============================================================

\section{Lesson 35: The pipeline in three commands}

\textbf{One-time setup ($\sim 20$ minutes):}
\begin{verbatim}
python3 finetune_rotated_mnist.py --epochs 5
\end{verbatim}
Fine-tunes the pretrained diffusion model on rotated MNIST and saves it.

\textbf{Stage A --- PnP-DM Split-Gibbs ($\sim 7$ minutes):}
\begin{verbatim}
python3 pnp_dm.py --ddpm_path ./ddpm_mnist_rot15 \
                  --n_chains 32 --n_iter 80 \
                  --out_mat ../pnp_dm_v3_answer.mat
\end{verbatim}
Runs 32 chains for 80 Gibbs iterations each. Picks the chain with the
lowest misfit and saves it.

\textbf{Stage B --- Gauss-Newton CG polish ($\sim 25$ seconds):}
\begin{verbatim}
python3 gn_cg_polish.py --init_mat ../pnp_dm_v3_answer.mat \
                        --K 500 --n_cg 150 \
                        --out_mat ../final_answer.mat
\end{verbatim}
Drives the misfit down to the float64 floor and saves the final answer.

\section{Lesson 36: The result on the competition vector}

Running this on the professor's competition vector:

\begin{itemize}[leftmargin=2em]
\item Recovered digit: \textbf{5}, rotated by about $7.5^\circ$.
\item Final misfit: $\mathbf{6.16 \times 10^{-13}}$.
\end{itemize}
...
visible. The simulator $F$ applied to the right gives back something
matching the left to about 13 decimal places.

\section{Lesson 37: Held-out validation}

To check the pipeline isn't overfit to the competition vector, we ran
it on 10 unseen MNIST test digits (one per class 0-9). The diffusion
model was fine-tuned on the training set only; test digits are
genuinely unseen.
...
Result: \textbf{9 of 10 correct}.

\begin{center}
\includegraphics[width=0.99\textwidth]{validate_final.png}
\end{center}

The lone failure was digit 3 (recovered as a 5-shaped image). The
reason: that particular test image's hand-drawn 3 has a tightly curled
bottom that, under the ERT physics, produces voltage readings
extremely similar to those of certain hand-drawn 5s. The two digits are
genuinely physically indistinguishable through 1900 boundary
measurements. That's an ambiguity of the \emph{problem}, not of our
method.

All 9 correctly-recovered images polished to misfits between $6.4
\times 10^{-13}$ and $7.6 \times 10^{-12}$ --- the same float64 regime
as the competition vector. The pipeline is robust.

\section{Lesson 38: A 60-second explanation you can give a stranger}
...
diffusion model knows what digits look like. We first fine-tuned it on
rotated MNIST so it would believe in slightly tilted digits, because
our truth was tilted. Then we ran a Bayesian sampler that alternates
two moves: a Gauss-Newton step that pulls the candidate image toward
fitting the data, and a one-step diffusion denoise that pulls it
toward looking like a digit. We ran 32 of these chains in parallel;
all 32 converged on a 5. Finally we polished the best chain with
Newton's method, using conjugate gradient to solve the inner linear
systems, until the residual was indistinguishable from zero in double
precision. The whole pipeline takes about 8 minutes after a one-time
training, and the final misfit was about $6 \times 10^{-13}$.''
\end{intuition}

\section{Lesson 39: Glossary}
...
\item \textbf{Rotation augmentation.} Fine-tune the diffusion model
  on rotated MNIST so it knows tilted digits are realistic. (Lesson 26.)
...
\item \textbf{Linear convergence.} Each step shrinks the error by a
  constant factor (slow near the optimum). (Lesson 28.)
\item \textbf{Quadratic convergence.} Each step roughly squares the
  error (very fast near the optimum). (Lesson 29.)
...
\item \textbf{Line search.} Shrink the step length until the misfit
  actually decreases. (Lesson 33.)
\item \textbf{Trust-region damping.} Grow or shrink the damping
  parameter based on whether the last step succeeded. (Lesson 33.)
\item \textbf{Float64 floor.} The misfit below which double-precision
  ...
  10^{-13}$ for our problem. (Lesson 34.)
\end{itemize}
