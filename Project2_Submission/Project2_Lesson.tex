...
\item Pre-polish misfit: $\mathbf{9.59 \times 10^{-4}}$.
\item Classifier confidence on the recovered image: $0.98$.
\end{itemize}
...
$9.7 \times 10^{-4}$, with the lowest belonging to the chain seeded
to class 1 (final misfit $9.515 \times 10^{-4}$, but the recovered
image reads as a 2 with $0.98$ confidence because the data step
overrode the CFG class hint). Running TV polish on top of this drives
the polished misfit \emph{up} to $1.23 \times 10^{-3}$ as TV pushes
the image toward smoother solutions that fit the data less well: the
TV-polish update was tuned for the $10^{-11}$ regime of the noiseless
example, and on noisy data it overshoots. The pre-polish image is the
answer.
...
\item \textbf{Final misfits.} $1.83 \times 10^{-11}$ on the noiseless
  digit-5 worked example (polished, float64 floor); $9.59 \times
