...
Result: \textbf{9 of 10 correct} on the base pipeline. We did not
re-run the held-out sweep with the new (class-conditional + CFG + TV)
pipeline because each held-out digit costs about ten minutes; the
visual comparison on the competition vector (Lesson 41) is the
evidence we have for the new pipeline's image-quality improvement.
...
method. With the new class-conditional pipeline, the chain assigned
class label 3 would commit to a 3 from the start, so we would at
least see a 3-shaped recovery in the chain pool; whether that chain
also produces the lowest data misfit is the open question.
