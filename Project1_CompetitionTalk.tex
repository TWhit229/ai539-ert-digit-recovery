...
\small
Let $\mathcal{D} = \{m_1, \ldots, m_N\}$ be the $N = 60{,}000$ MNIST training
images, $\sigma_b = 1$ the background, $F$ the ERT forward map, and
$y_{\text{obs}}$ the measurements. Define the data misfit
\[
...
\[
i^{\star} \;=\; \arg\min_{i \in \{1, \ldots, N\}} L(m_i), \qquad x^{(0)} \;=\; m_{i^{\star}} .
\]

\textbf{Stage 2 (warm-started refinement).} With $\theta^{(0)} = \mathbf{0}$, for $k = 0, 1, \ldots, K-1$:
\begin{align*}
...
\end{align*}
Return $\hat{\sigma} = \sigma_b\,\mathbf{1} + x^{(K)}$.

\vspace{0.3em}
{\footnotesize\textcolor{osugray}{No explicit regularizer in $L$. The prior is the discrete dictionary $\mathcal{D}$ itself.}}
\end{frame}
