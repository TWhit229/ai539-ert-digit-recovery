...
%-------------------------------------------------------------
\begin{frame}{The method, in math}
\footnotesize
$\mathcal{D} = \{m_1, \ldots, m_N\}$: the $N = 60{,}000$ MNIST training images.
$\sigma_b = 1$: background. $F$: ERT forward map. $y_{\text{obs}}$: measurements.

\smallskip
\begin{align*}
L(x) \;&=\; \tfrac{1}{2}\, \bigl\| F(\sigma_b\,\mathbf{1} + x) - y_{\text{obs}} \bigr\|_2^2
       & &\textcolor{osuorange}{\leftarrow\ \text{misfit: how wrong are the readings?}}
\end{align*}

\textbf{Stage 1 (template matching).}
\begin{align*}
i^{\star} \;&=\; \arg\min_{i \in \{1, \ldots, N\}} L(m_i)
       & &\textcolor{osuorange}{\leftarrow\ \text{index of the best of all 60{,}000 templates}} \\
x^{(0)} \;&=\; m_{i^{\star}}
       & &\textcolor{osuorange}{\leftarrow\ \text{starting image for Stage 2}}
\end{align*}

\textbf{Stage 2.} With $\theta^{(0)} = \mathbf{0}$, for $k = 0, 1, \ldots, K-1$:
\begin{align*}
r^{(k)} \;&=\; F(\sigma_b\,\mathbf{1} + x^{(k)}) - y_{\text{obs}}
       & &\textcolor{osuorange}{\leftarrow\ \text{residual at step } k} \\[0.4em]
\theta^{(k+1)} \;&=\; \underbrace{\gamma\,\theta^{(k)}}_{\textcolor{osuorange}{\text{momentum (keep 95\% of last)}}}
                    + \underbrace{\eta\, J(x^{(k)})^{\top}\, r^{(k)}}_{\textcolor{osuorange}{\text{gradient step}}}
       & &\textcolor{osuorange}{\leftarrow\ \text{new velocity}} \\[0.4em]
x^{(k+1)} \;&=\; \max\bigl(x^{(k)} - \theta^{(k+1)},\; -\sigma_b + \varepsilon\bigr)
       & &\textcolor{osuorange}{\leftarrow\ \text{nudge image, clamp so } \sigma > 0}
\end{align*}

\smallskip
$\textbf{Return } \hat{\sigma} = \sigma_b\,\mathbf{1} + x^{(K)}$
\hspace{0.6em}\textcolor{osuorange}{$\leftarrow$ final conductivity image (background + final pixels).}
\end{frame}
...
%-------------------------------------------------------------
\begin{frame}{Same method on 10 unseen digits: 9 of 10 correct}
\centering
\includegraphics[width=0.95\linewidth]{validation_slide.png}

\vspace{0.6em}
\small
Labels read \textbf{true $\rightarrow$ recovered}. The one miss (5 $\rightarrow$
6) is a handwritten 5 with a looped bottom that resembles a 6, and it was the
least confident match. The competition data is not a close call: the top eight
matches were almost all 5's.
\end{frame}
